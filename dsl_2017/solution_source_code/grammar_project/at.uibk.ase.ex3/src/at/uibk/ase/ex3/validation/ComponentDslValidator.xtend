...
class ComponentDslValidator extends AbstractComponentDslValidator {
	...
	def checkDependencyAssignmentSource(AddDependency addDependency) {
		if (addDependency.source.entity.parent == EntityBaseTypeEnum.GROUP) {
	...
	def checkDependencyAssignmentDestination(AddDependency addDependency) {
		if (addDependency.dest.entity.parent == EntityBaseTypeEnum.GROUP) {
	...
	def checkAssignmentSourceType(Add addAction) {
		if (addAction.source.entity.parent == EntityBaseTypeEnum.GROUP) {
	...
	def checkAssignmentDestType(Add addAction) {
		if (addAction.dest.entity.parent == EntityBaseTypeEnum.MEMBER) {
	...
	def getAllProjects(List<Object> candidates) {
		...
		for (c : candidates) {
			if (c instanceof EntityType) {
				...
				if (proj.entity.parent == EntityBaseTypeEnum.MEMBER && proj.name !== null) {
					projects.add(c)
				}
			}
		}
	...
	def getAllComponents(List<Object> candidates) {
		...
		for (c : candidates) {
			if (c instanceof EntityType) {
				val proj = c as EntityType
				if (proj.entity.parent == EntityBaseTypeEnum.GROUP && proj.name !== null) {
					components.add(c)
				}
			}
		}
	...
	def validateExistingGroupVersions(Create createAction, List<Object> candidates) {
			...
			for (f : components){
				if (!p.identityEquals(f)){
					if (p.name.equals(f.name) && p.version.equals(f.version)){
						//FIX ME: correct CREATe action in the grammar, collect all the entities from this action
						//error(INVALID_CREATION_GROUP_VERSION, ComponentDslPackage.eINSTANCE.add_Source,INVALID_NAME)
						println(p.name + " " + p.version + " == "+ f.name + " "+ f.version)
						println(p.name+": "+ INVALID_CREATION_GROUP_VERSION)
					}
				}
			}
		}
	...
	def validateExistingMemberVersions(Create createAction, List<Object> candidates) {
		...
		for (p : projects) {
			for (f : projects){
				if (!p.identityEquals(f)){
					if (p.name.equals(f.name) && p.version.equals(f.version)){
						println(p.name + " " + p.version + " == "+ f.name + " "+ f.version)
						println(p.name+": "+ INVALID_CREATION_MEMBER_VERSION)
					}
				}
			}
		}
	...
	def checkCreateNewVersions(Create createAction) {
		...
		validateExistingMemberVersions(createAction, allEntities)
		validateExistingGroupVersions(createAction, allEntities)
	}
	...
	def checkUsage(Size sizeAction) {
		if (sizeAction.source.entity.parent == EntityBaseTypeEnum.MEMBER) {
	...
	def checkUsage(Order orderAction) {
		if (orderAction.source.entity.parent == EntityBaseTypeEnum.MEMBER) {
	...
	def checkUsage(Cycle cycleAction){
		if (cycleAction.source.entity.parent == EntityBaseTypeEnum.MEMBER){
			error(INVALID_ACTION_USAGE, ComponentDslPackage.eINSTANCE.cycle_Source, INVALID_NAME)
			return
		}
		if (cycleAction.dest.entity.parent == EntityBaseTypeEnum.MEMBER){
